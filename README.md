...
командой через виджет), адрес и порт RS-485. Параметры самого порта не
настраиваются — протокол Dauerhaft PRO фиксирован (9600 8N1). Интервал
проверки связи (мс) и отладочные сообщения — опциональные, добавляются
через кнопку «Свойства» редактора.
