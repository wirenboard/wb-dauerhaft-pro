...
виртуальное устройство: движение и ползунок позиции, индикаторы незаданных
пределов хода, промежуточная точка, угол ламелей (для моделей с ламелями), смена
адреса на шине и индикация доступности.
...
Сервис `wb-dauerhaft-pro` стартует автоматически после установки. Конфигурация
по умолчанию — пустой список приводов, поэтому сразу после установки сервис
завершается с кодом 7 («нечего делать»); systemd считает такой выход успешным,
и юнит показывается как `inactive`, а не `failed`. Работать сервис начнёт после
того, как в конфигурацию добавят хотя бы один привод и нажмут «Записать». По
кнопке «Записать» в редакторе конфигураций он перезапускается — правки сразу
видны в разделе «Устройства».
...
- **Верхний предел не задан** / **Нижний предел не задан** — индикаторы только
  для чтения: 1, пока соответствующий предел хода у привода не задан, 0 — предел
  задан;
...
зеркалит позицию — и показ, и команду ползунка. Индикаторы пределов реверс не
меняет: «верхний» и «нижний» — в терминах самого привода, как в инструкции
производителя по установке пределов.
...
после исправления конфигурации сервис перезапускается кнопкой «Записать». Пустой список приводов — код 7 (нечего делать):
так завершается и первый запуск сразу после установки, с конфигурацией по умолчанию; systemd считает этот код успехом
(`SuccessExitStatus=7`), поэтому юнит остаётся `inactive`, а не `failed`. Отказ брокера в авторизации — код 2.
Недоступный брокер демон ждёт, не завершаясь; после его перезапуска состояние
приводов и подписки восстанавливаются. По SIGINT/SIGTERM демон удаляет свои retained-топики (если брокер доступен) и выходит с кодом 0.

## Ссылки

Сайт производителя: [Dauerhaft RS PRO](https://pfkz.ru/dauerhaft_rs_pro)
(раздвижные карнизы) и [Dauerhaft ROLL RS PRO](https://pfkz.ru/dauerhaft_roll_rs_pro)
(рулонные).
