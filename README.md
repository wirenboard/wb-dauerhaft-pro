...
отправляет команды в выбранный порт. Опциональные поля добавляются через
кнопку «Свойства» редактора: у привода — шкала угла ламелей (включает
контролы угла: прямая 0–180° или сжатая, 0–180° в 36–144), на уровне
драйвера — интервал проверки связи (мс) и отладочные сообщения.
