...
# answers slower than a plain move, so these operations get a longer timeout.
FLASH_TIMEOUT_MS = 5000

# expect_address value that accepts a reply from any bus address — for a
# learning address write, where the answering motor's address is not known in
# advance.
ANY_ADDRESS = -1
...
class Actuator:
    ...
    def set_address(self, new_address: int) -> bool:
        """
        Change THIS device's address (unicast, to its current address).

        Only this one device changes. The reply comes from the new address (spec
        3.1). On success the runtime config follows the new address so control
        keeps working, and a warning is logged — the on-disk config must be
        updated to persist it.
        """
    ...
    def set_address_learning(self, new_address: int) -> bool:
        ...
        A safe way to address a motor on a shared bus without knowing its current
        address; times out when no motor is in learning mode. The runtime address
        is not touched — the ack proves some motor took the address, not that it
        was this one, so the config stays the source of truth.
        """
    ...
    def _change_address(self, target_address: int, new_address: int, follow: bool) -> bool:
        ...
        *follow* carries three linked meanings, all true only for a unicast
        ...
        resp = self._exchange(
            ...
            # its ack (or refusal) from an address not known in advance, so the
            # address filter is off there (expect_address=ANY_ADDRESS). A timeout
            # is the EXPECTED outcome of a learning write with no motor in its
            # window, so these ops do not count toward this actuator's offline
            # hysteresis (count_misses=False).
