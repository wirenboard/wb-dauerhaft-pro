...
read position and angle, and set the device address (unicast, broadcast or via
the button-learning window). This module is pure (no I/O) so it can be
...
def set_address(address: int, new_address: int) -> bytes:
    ...
    :data:`BROADCAST_ADDRESS` (all motors), or to :data:`LEARNING_ADDRESS` for
    a motor put into independent-setting mode by its button (section 3.1).
    """
