...
code and a visible MQTT announcement, an empty device list clears that
announcement and ends with 7, a rejected MQTT login ends with 2, a stop signal
while the broker is still awaited ends with 0, an unusable broker URL is an
argument error (2), and the journald stream detection does not misfire.
"""
...
class FakeMQTTClient:
    ...
    def wait_for_connection(self, stop_requested):
        """
        Polls every 50 ms and returns False as soon as a stop is requested, even
        when the fake counts as connected (start() ran, stop() did not); True
        once it is connected. wb-common's real method checks is_connected()
        first and returns True even with the stop event set, so the two differ
        exactly in the "connected and stopped at once" race.
        """
...
@pytest.mark.parametrize("url", ["tcp://host", "foo://x", "tcp://user:secret@host:none"])
def test_unusable_broker_url_is_an_argument_error(monkeypatch, capsys, url):
    """
    A broker URL wb-common's client cannot use (no port, unknown scheme, non-numeric
    port) is an argument error: argparse exits with 2 and a usage message, before any
    MQTT client exists — and the message never echoes the URL, which may carry a password.
    """
    FakeMQTTClient.instances.clear()
    monkeypatch.setattr(main_mod, "MQTTClient", FakeMQTTClient)
    monkeypatch.setattr("sys.argv", ["wb-dauerhaft-pro", "-b", url])
    with pytest.raises(SystemExit) as exc:
        main_mod.main()
    assert exc.value.code == main_mod.EXIT_INVALIDARGUMENT
    assert not FakeMQTTClient.instances
    err = capsys.readouterr().err
    assert "broker URL must be" in err and "secret" not in err
...
def test_rejected_login_exits_invalidargument(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(main_mod.signal, "signal", {}.__setitem__)  # record, do not install, the handlers
...
def test_signal_while_waiting_for_the_broker_exits_success(tmp_path, monkeypatch, caplog):
    """
    With the broker down the daemon waits instead of exiting with 1; its
    SIGTERM handler fired during that wait ends it with 0 and a log line saying
    the retained topics could not be removed.
    """
    ...
    monkeypatch.setattr("sys.argv", ["wb-dauerhaft-pro", "-c", _write_config(tmp_path, ONE_DEVICE)])
    handlers = {}  # main() records its SIGINT/SIGTERM handlers here instead of installing them
    monkeypatch.setattr(main_mod.signal, "signal", handlers.__setitem__)
    # SIGTERM arrives while main() waits for the broker: run the handler it has registered by then
    threading.Timer(0.1, lambda: handlers[signal.SIGTERM](signal.SIGTERM, None)).start()
