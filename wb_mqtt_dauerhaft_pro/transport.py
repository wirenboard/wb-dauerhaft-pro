...
``modbus-utils-rpc`` / ``wb-mqtt-dali``)::
    ...
    params = {path, baud_rate, parity, data_bits, stop_bits,
              format="HEX", msg=<hex>, response_size, total_timeout[, response_timeout]}
    -> {"response": "<hex reply>"}
...
# everything that depends on it (device model, tests) — imports on a dev box that
# has no paho/mqttrpc installed. The daemon runs on the controller where both exist.

DEFAULT_BROKER_HOST = "127.0.0.1"
DEFAULT_BROKER_PORT = 1883

# Single request/response exchange budget. The spec promises a reply in <10 ms;
# this leaves generous headroom for bus scheduling inside wb-mqtt-serial.
...
def connect(
    ...
    **kwargs,
    ...
    Sets up a paho MQTT client, wires the mqttrpc reply handler, runs the network
    loop in the background, and tears everything down on exit.
    """
